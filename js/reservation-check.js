...
async function checkReservations() {
    ...
    try {
        console.log('예약 조회 시작:', { nameInput, codeInput });
        
        // 타임과 술자 데이터 로드
        const timesResponse = await getData('times', { limit: 100 });
        allTimes = (timesResponse && timesResponse.data) ? timesResponse.data : [];
        
        const operatorsResponse = await getData('operators', { limit: 100 });
        allOperators = (operatorsResponse && operatorsResponse.data) ? operatorsResponse.data : [];
        
        // 예약 데이터 로드
        let reservationsResponse;
        try {
            reservationsResponse = await getData('reservations', { limit: 100 });
            currentReservations = (reservationsResponse && reservationsResponse.data) ? reservationsResponse.data : [];
        } catch (error) {
    ...
    currentReservations.forEach(reservation => {
        ...
        html += `
            <tr style="${rowStyle}">
                ...
                <td style="max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${reservation.participant_address || '-'}">${reservation.participant_address || '-'}</td>
...
function displayRSView() {
    ...
    // RS는 제한된 정보만 표시: 타임, 술자명, 날짜, 대상자 이름, 관계
    let html = `
        <div style="overflow-x: auto;">
        ...
        html += `
            <div class="time-card" style="${cardStyle}">
                ...
                <div style="background-color: #f8f9fa; padding: 20px; border-radius: 5px; margin-bottom: 15px;">
                    ...
                    <p style="margin: 5px 0;"><strong>대상자 이름:</strong> ${reservation.participant_name}</p>
                    ...
                    <p style="margin: 5px 0;"><strong>전화번호:</strong> ${reservation.participant_phone}</p>
