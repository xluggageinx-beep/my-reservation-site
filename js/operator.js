...
async function loadTimes() {
    ...
    try {
        ...
        const response = await getData('times', { limit: 100 });
        console.log('타임 응답:', response);
        
        times = (response && response.data) ? response.data : [];
...
function displayTimes() {
    ...
    times.forEach(time => {
        ...
        const datesCount = time.selected_dates ? time.selected_dates.length : 0;
...
function editTime(timeId) {
    ...
    selectedDatesForTime = currentEditingTime.selected_dates || [];
...
async function deleteTime(timeId) {
    // 해당 타임에 속한 술자가 있는지 확인
    const operatorsResponse = await getData('operators', { limit: 1000 });
    const allOperators = operatorsResponse.data || [];
...
async function loadReservationsSummary() {
    try {
        ...
        const response = await getData('reservations', { limit: 100 });
        const reservations = (response && response.data) ? response.data : [];
        ...
        if (summaryContainer) {
            if (reservations.length === 0) {
                ...
                // 타임 데이터와 술자 데이터 로드
                const timesResponse = await getData('times', { limit: 100 });
                const allTimes = (timesResponse && timesResponse.data) ? timesResponse.data : [];
                
                const operatorsResponse = await getData('operators', { limit: 100 });
                const allOperators = (operatorsResponse && operatorsResponse.data) ? operatorsResponse.data : [];
